...
exports.renewBorrowal = exports.returnBorrowedBook = exports.rejectBorrowRequest = exports.approveBorrowRequest = exports.borrowBook = exports.getAllBorrowals = exports.getUserBorrowals = void 0;
...
const getUserBorrowals = async (req, res) => {
    try {
        const userId = req.user.id;
        const { data: borrowals, error } = await supabase_1.supabase
            .from('borrowals')
            .select('*, book:books(*)')
            .eq('user_id', userId)
...
exports.getUserBorrowals = getUserBorrowals;
...
exports.getAllBorrowals = getAllBorrowals;
...
const borrowBook = async (req, res) => {
    try {
        ...
        const { data: borrowal, error } = await supabase_1.supabase
            .from('borrowals')
            .insert([{
                user_id: userId,
                book_id: finalBookId,
        ...
        if (io)
            io.emit('book_requested', { bookId: finalBookId, userId });
...
const rejectBorrowRequest = async (req, res) => {
    try {
        ...
        const { data: deleted, error } = await supabase_1.supabase
            .from('borrowals')
            .delete()
            .eq('id', id)
        ...
        const io = req.app.get('io');
        if (io)
            io.emit('request_rejected', { borrowalId: id, bookId: deleted.book_id, userId: deleted.user_id, reason });
...
// User: Renew borrowal
const renewBorrowal = async (req, res) => {
    try {
        const { id } = req.params;
        const dueDate = new Date();
...
exports.renewBorrowal = renewBorrowal;
