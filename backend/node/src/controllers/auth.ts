...
export const registerAdmin = async (req: Request, res: Response) => {
  try {
    const { email, password, fullName, role, adminInviteCode } = req.body as AdminRegisterBody;
...
export const getMyRole = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user?.id) {
      return res.status(401).json({ error: 'Authentication required' });
    }

    const normalize = (rawRole: unknown): 'admin' | 'user' =>
      String(rawRole || '').trim().toLowerCase() === 'admin' ? 'admin' : 'user';
