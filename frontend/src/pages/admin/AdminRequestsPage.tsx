...
export default function AdminRequestsPage() {
  ...
  return (
    <>
      <Helmet><title>Requests — Admin — LibraVault</title></Helmet>
      <div className="mb-6">
        <Badge variant="blue" size="md" className="mb-2">Admin</Badge>
        <h1 className="font-heading font-bold text-3xl uppercase">Borrowal Requests</h1>
      </div>
      ...
      {reqQuery.isLoading ? (
        ...
        <Card color="coral" padding="lg" hoverable={false}>
          <p className="font-heading font-bold uppercase text-white">Failed to load requests.</p>
        </Card>
          ...
          {(items as any[]).map((r) => (
            <Card key={r.id} color="white" padding="md" hoverable={false}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <p className="font-heading font-bold uppercase">{r.books?.title || r.book?.title || 'Request'}</p>
                  <p className="text-sm text-brutal-dark-gray">
                    User: <span className="font-bold">{r.user_id || '—'}</span>
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="success" onClick={() => approve.mutate(r.id)} isLoading={approve.isPending}>
                    Approve
                  </Button>
                  <Button size="sm" variant="danger" onClick={() => reject.mutate(r.id)} isLoading={reject.isPending}>
                    Reject
                  </Button>
                </div>
              </div>
